...
export default async function ProductIdRoute({
  ...
  const content = data?.description ? JSON.parse(JSON.stringify(data.description)) : null;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start lg:gap-x-24 py-6">
        ...
        <div className="col-span-1 sticky top-10">
          ...
          <div className="flex items-end ">
            <p className="line-through text-lg">{formatCurrency(data.discountAmount as number)}</p>
            <p className="text-3xl mt-2 font-semibold text-[#ad0577]">{formatCurrency(data.price)}</p>
          </div>
          ...
          <div>
            <p className="flex items-center gap-2"><IoCheckmarkOutline size={22} color="green"/>Plata sigura</p>
            <p className="flex items-center gap-2"><IoCheckmarkOutline size={22} color="green"/>Livrare sigură</p>
            <p className="flex items-center gap-2"><IoCheckmarkOutline size={22} color="green"/>Retururi gratuite</p>
          </div>
